...
class SPPnet:
    ...
    def inference(self, data, train=True, num_class=1000):
        with tf.name_scope('Processing'):
            ...
            self.conv5_3 = self._conv_layer(self.conv5_2, 'conv5_3', [3,3,512,512])#<tf.Tensor 'Processing/conv5_3/Relu:0' shape=(32, 14, 14, 512) dtype=float32>
            
            bins = [3, 2, 1]
            map_size = self.conv5_3.get_shape().as_list()[2]#14
            print(self.conv5_3.get_shape())
            sppLayer = SPPLayer(bins, map_size)
            self.sppool = sppLayer.spatial_pyramid_pooling(self.conv5_3)#SPP processing shape=(32, 7168)
            ...
            self.fc8 = self._fc_layer(self.fc7, 'fc8', shape=[4096,num_class])#<tf.Tensor 'Processing/fc8/BiasAdd:0' shape=(32, 607) dtype=float32>
            print('inference')
            return self.fc8
